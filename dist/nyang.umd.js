...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.NYANG = factory(global.d3));
  ...
  String.prototype.width = function (textStyle) {
    ...
    var d = d3.select("body").append("div").attr("class", textStyle) //The font size defaults to 12px right now. This may need to be changed in the future.
  ...
  var ZoomHandler = /*#__PURE__*/function () {
    function ZoomHandler(graphContainerElement, eventEmitter, options) {
      ...
      this.zoom = d3.zoom().scaleExtent(Env.SCALE_EXTENT).on("zoom", function () {
    ...
    _createClass(ZoomHandler, [{
      ...
      value: function resetZoom() {
        ...
        d3.select(this.graphContainerElement).select("svg").transition().duration(Env.ZOOM_TIME / 4).call(this.zoom.scaleBy, newScale).transition().call(this.zoom.translateTo, midX, midY);
      }
      /**
      ...
      value: function zoomToCoordinates(x, y, scale) {
        d3.select(this.graphContainerElement).select("svg").transition().duration(Env.ZOOM_TIME).call(this.zoom.transform, d3.zoomIdentity.translate(x, y).scale(scale)).select("g").attr("transform", "translate(".concat(x, ",").concat(y, ")scale(").concat(scale, ")"));
      }
      /**
  ...
  var ContextMenu = /*#__PURE__*/function () {
    ...
    _createClass(ContextMenu, [{
      ...
      value: function createContextMenu(clickedItem, contextSectionsArray, customSectionsArray, mouseX, mouseY) {
        ...
        var ulElement = d3.select(this.graphContainerElement).append("div").attr("id", "nyang-context-menu-container").attr("class", "nyang-context-menu").style("position", "fixed").style("left", mouseX + "px").style("top", mouseY + "px").style("display", "block").append("ul").attr("class", "nyang-context-menu-options");
    ...
    _createClass(Highlighter, [{
      ...
      value: function writeHighlightFilters() {
        ...
        var shadowFilter = function shadowFilter(id, deviation, opacity) {
          d3.select(_this2.graphContainerElement).select("defs").append("filter").attr("id", id).append("feDropShadow").attr("dx", "0").attr("dy", "0").attr("stdDeviation", deviation).attr("flood-opacity", opacity);
        };
      ...
      value: function removeAllEntityFocus() {
        ...
        if (this.enableOnionOnFocus) {
          d3.selectAll(".onion-clone").attr("class", "removing").transition().duration(Env.DEFAULT_ONION_ANIMATION_TIME).style("transform", "scale(0.8)").remove();
        }
      }
      /**
      ...
      value: function toggleNodeEntityFocus(entityID) {
        var nodeElement = d3.select("[id='".concat(entityID, "']")); //html4 support
      ...
      value: function highlightNode(nodes) {
        ...
        }).append("circle").attr("r", 50).classed("highlighted-node", true).transition().duration(Env.HIGHLIGHT_TIME).ease(d3.easeBounce).style("transform", "scale(5)").transition().duration(Env.HIGHLIGHT_TIME_REMOVE).remove();
      }
      /**
  ...
  var DOMProcessor = /*#__PURE__*/function () {
    ...
    _createClass(DOMProcessor, [{
      ...
      value: function updateNodes(nodes) {
        ...
        nodes.forEach(function (node) {
          d3.select("[id='badge-".concat(node.id, "-hidden-edge-counter']")).remove();
        ...
        nodes.forEach(function (node) {
          d3.select("[id='pin-".concat(node.id, "']")).remove();
      ...
      value: function handleHoverEvent(hoveredData, eventType) {
        ...
        if (this.enableFadeOnHover) {
          if (!hoveredData.sourceNode) {
            ...
            }).transition().duration(Env.FADE_TIME).ease(d3.easeLinear).style("opacity", opacity);
            d3.selectAll(".edge").filter(function (d) {
  ...
  var UI = /*#__PURE__*/function () {
    ...
    _createClass(UI, [{
      ...
      value: function initializeDOM() {
        ...
        var rootG = d3.select(this.graphContainerElement).insert("svg", "*").attr("class", "nyang").classed("svgGraph", true).attr("width", "100%").attr("height", "100%").on("click", function () {
  ...
  var Engine = /*#__PURE__*/function () {
    ...
    _createClass(Engine, [{
      ...
      value: function initializeSimulation() {
        ...
        return d3.forceSimulation().force("charge", d3.forceManyBody().strength(Env.CHARGE)).force("center", d3.forceCenter(this.forceCenterX, this.forceCenterY)).force("y", d3.forceY(0).strength(Env.GRAVITY)).force("x", d3.forceX(0).strength(Env.GRAVITY)).nodes([]).force("link", d3.forceLink().links([]).distance(function (l) {
      ...
      value: function resetLayout(nodes, edges) {
        ...
        this.simulation.force("y", d3.forceY(0).strength(Env.GRAVITY)).force("x", d3.forceX(0).strength(Env.GRAVITY)).force("link", d3.forceLink().links(edges).distance(function (l) {
          return _this5.getEdgeDistance(l);
        }).strength(Env.EDGE_STRENGTH)).force("charge", d3.forceManyBody().strength(Env.CHARGE));
      }
      /**
       * Returns the distance (length) of the passed edge
       * @param {object} l - Edge
