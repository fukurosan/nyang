...
class Datastore {
  constructor(nodes, edges, eventEmitter) {
    ...
    this.ee.on(_EventEnum.default.DATA_UPDATE_REQUESTED, (nodes, edges) => this.updateDataset(nodes, edges));
    ...
    this.ee.on(_EventEnum.default.DATA_FILTER_REQUESTED, filters => {
      this.setFilters(filters);
      this.applyFilters();
      this.updateNumberOfHiddenEdgesOnNodes();
      this.updateLiveData();
    });
    this.ee.on(_EventEnum.default.DATA_FILTER_RESET_REQUESTED, () => {
    ...
    this.ee.on(_EventEnum.default.IMPLODE_EXPLODE_REQUESTED, (id, isImplode) => {
      this.implodeOrExplodeNode(id, isImplode);
      this.updateNumberOfHiddenEdgesOnNodes();
      this.updateLiveData();
    });
    this.ee.on(_EventEnum.default.IMPLODE_EXPLODE_LEAFS_REQUESTED, (id, isImplode) => {
      this.implodeOrExplodeNodeLeafs(id, isImplode);
      this.updateNumberOfHiddenEdgesOnNodes();
      this.updateLiveData();
    });
    this.ee.on(_EventEnum.default.IMPLODE_EXPLODE_RECURSIVE_REQUESTED, (id, isImplode) => {
      this.implodeOrExplodeNodeRecursive(id, isImplode);
      this.updateNumberOfHiddenEdgesOnNodes();
      this.updateLiveData();
    });
    this.ee.on(_EventEnum.default.IMPLODE_EXPLODE_NON_CIRCULAR_REQUESTED, (id, isImplode) => {
      this.implodeOrExplodeNodeNonCircular(id, isImplode);
      this.updateNumberOfHiddenEdgesOnNodes();
      this.updateLiveData();
    });
    this.updateEdgeIDs(edges);
    this.applyFilters();
    this.updateNumberOfHiddenEdgesOnNodes();
  }
  ...
  updateLiveData() {
    ...
    const edges = this.allEdges.filter(edge => this.isEdgeLive(edge)); //Apply the result to the live data
  ...
  implodeOrExplodeNodeLeafs(rootNodeID, isImplode) {
    const connectedEdges = this.allEdges.filter(edge => edge.sourceNode === rootNodeID).filter(edge => !this.allEdges.find(secondaryEdge => secondaryEdge.sourceNode === edge.targetNode));
  ...
  implodeOrExplodeNodeRecursive(nodeID, isImplode) {
    let processedNodeIDs = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];

    if (!processedNodeIDs.includes(nodeID)) {
      processedNodeIDs.push(nodeID);
      const {
        updatedNodes
      } = this.implodeOrExplodeNode(nodeID, isImplode);
  ...
  */
  ...
  implodeOrExplodeNodeNonCircular(rootNodeID, isImplode) {
    const connectedEdges = this.allEdges.filter(edge => edge.sourceNode === rootNodeID).filter(edge => this.calculateEdgePathFromNodeToNode(edge.targetNode, rootNodeID).length === 0);
    const connectedNodes = connectedEdges.map(edge => edge.targetNode);
    const collateralEdges = this.allEdges.filter(edge => connectedNodes.includes(edge.sourceNode) || connectedNodes.includes(edge.targetNode));
    const edges = [...connectedEdges, ...collateralEdges];
    const nodes = connectedNodes.map(nodeID => this.getNodeByID(nodeID));
  ...
  calculateEdgePathFromNodeToNode(nodeIDFrom, nodeIDTo) {
    let path = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
    let crossedNodes = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
    let result = [];
