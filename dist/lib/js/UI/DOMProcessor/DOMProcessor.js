...
class DOMProcessor {
  constructor(rootG, eventEmitter, userDefinedOptions) {
    this.enableMultiLineNodeLabels = userDefinedOptions.enableMultiLineNodeLabels !== undefined ? userDefinedOptions.enableMultiLineNodeLabels : _Env.default.DEFAULT_NODE_TEXT_MULTILINE;
    this.enableFadeOnHover = userDefinedOptions.enableFadeOnHover !== undefined ? userDefinedOptions.enableFadeOnHover : _Env.default.DEFAULT_FADE_ON_HOVER;
    this.rotateLabels = userDefinedOptions.rotateLabels !== undefined ? userDefinedOptions.rotateLabels : _Env.default.ROTATE_LABELS;
    ...
    this.ee.on(_EventEnum.default.TOGGLE_MULTIPLICITY_REQUESTED, () => {
      this.showMultiplicity = !this.showMultiplicity;
      this.updateMultiplicityCounters(this.edges);
    });
    this.ee.on(_EventEnum.default.DATA_PROCESSOR_FINISHED, (nodes, edges) => {
      this.nodes = nodes;
      this.edges = edges; //The order of these matters, don't rearrange
      ...
      this.ee.trigger(_EventEnum.default.DOM_PROCESSOR_FINISHED, nodes, edges);
    });
    this.ee.on(_EventEnum.default.ENGINE_TICK, () => {
  ...
  updateEdges(edges) {
    ...
    const selector = this.rootG.select("#edge-container").selectAll(".edge").data(edges, d => {
      return d.id;
    });
    selector.exit().remove();
    selector.enter().append("g").attr("class", d => {
  ...
  updateMultiplicityCounters(edges) {
    ...
    if (this.showMultiplicity) {
      const selector = this.rootG.select("#multiplicity-container").selectAll(".multiplicity").data(edges, d => {
        return d.id;
      });
      selector.exit().remove();
  ...
  updateNodes(nodes) {
    /* Create node groups */
    const selector = this.rootG.select("#node-container").selectAll(".node").data(nodes, d => {
      return d.id;
    });
    selector.exit().remove();
    selector.enter().append("g").attr("class", "node").call(d3.drag().on("start", d => {
      //Stop force on start in case it was just a simple click
      this.ee.trigger(_EventEnum.default.NODE_DRAG_START, d);
      d.fx = d.x;
      d.fy = d.y;
    }).on("drag", d => {
      //Restart force on drag
      this.ee.trigger(_EventEnum.default.NODE_DRAG_DRAGGED, d);
      d.fx = d3.event.x;
      d.fy = d3.event.y;
    }).on("end", d => {
      d.fx = null;
      d.fy = null;
      this.ee.trigger(_EventEnum.default.NODE_DRAG_ENDED, d);
    })).each((d, i, c) => {
    ...
    nodes.forEach(node => {
      ...
      if (node.hiddenEdgeCount) {
        const element = d3.select("[id='".concat(node.id, "']")).select(function () {
  ...
  updateLabels(edges) {
    /* Create label groups */
    const selector = this.rootG.select("#label-container").selectAll(".label").data(edges, d => {
      return d.id;
    });
    selector.exit().remove();
  ...
  drawLabel(edge, data, direction) {
    const label = edge.append("g").attr("id", "label" + data.id + direction).classed(direction, true);
    this.drawLabelRect(label, data, direction);
    const labelText = label.append("text").attr("class", function () {
      //Generated class
  ...
  drawLabelRect(label, data, direction) {
    ...
    }).attr("x", -width / 2).attr("y", -_Env.default.LABEL_HEIGHT / 2).attr("width", width).attr("height", _Env.default.LABEL_HEIGHT).on("mouseenter", edgeData => {
  ...
  labelMouseEnter(edgeData, direction) {
    ...
    setTimeout(() => {
      ...
      if (marker.classed("hovered")) {
        //Handle event
        this.handleHoverEvent(edgeData, "enter"); //Sort the labels which brings the hovered one to the foreground
  ...
  drawMultiplicity(edge, direction) {
    const card = edge.append("g").attr("class", l => {
  ...
  drawNode(element, data) {
    switch (data.shape) {
      case "circle":
        element.append("circle").attr("r", d => d.radius).attr("class", "node-".concat(data.type ? data.type : "default")) //Generated Class
      ...
      case "layeredCircle":
        ...
        element.append("circle").attr("r", d => d.radius).attr("style", "stroke-width:2;fill:#fff;stroke:#000;stroke-dasharray:0;").attr("id", data.id);
        element.append("circle").attr("r", d => d.radius - 4).attr("class", "node-".concat(data.type ? data.type : "default")); //Generated Class
        //.append("svg:title")
        //.text((d) => d.name)
      ...
      case "rectangle":
        element.append("rect").attr("x", d => -d.width / 2).attr("y", d => -d.height / 2).attr("width", d => d.width).attr("height", d => d.height).attr("class", "node-".concat(data.type ? data.type : "default")) //Generated Class
        .attr("id", data.id); //.append("svg:title")
        //.text((d) => d.name)
    ...
    .on("mousemove", d => {
      this.ee.trigger(_EventEnum.default.MOUSE_OVER_NODE, d);
    }).on("mouseout", d => {
      this.ee.trigger(_EventEnum.default.MOUSE_LEFT_NODE, d);
    }); //draw textblock
  ...
  drawNodeCollapsedEdgeCounter(element, data) {
    const count = "".concat(data.hiddenEdgeCount);
  ...
  attachEntityClickListeners() {
    ...
    let timeout = null;
    this.rootG.selectAll(".node").on("click", d => {
      ...
      d3.event.stopPropagation();
      this.ee.trigger(_EventEnum.default.RIGHT_CLICK_ENTITY, d);
    });
    this.rootG.selectAll(".label .from").on("click", d => {
      d3.event.stopPropagation();
      clearTimeout(timeout);
      timeout = setTimeout(() => {
        this.ee.trigger(_EventEnum.default.CLICK_ENTITY, {
      ...
      d3.event.preventDefault();
      d3.event.stopPropagation();
      this.ee.trigger(_EventEnum.default.RIGHT_CLICK_ENTITY, d, "from");
    });
    this.rootG.selectAll(".label .to").on("click", d => {
      d3.event.stopPropagation();
      clearTimeout(timeout);
      timeout = setTimeout(() => {
        this.ee.trigger(_EventEnum.default.CLICK_ENTITY, {
      ...
      d3.event.preventDefault();
      d3.event.stopPropagation();
      this.ee.trigger(_EventEnum.default.RIGHT_CLICK_ENTITY, d, "to");
    });
  }
  ...
  handleHoverEvent(hoveredData, eventType) {
    ...
    if (this.enableFadeOnHover) {
      ...
      if (!hoveredData.sourceNode) {
        ...
        const opacity = eventType === "enter" ? "" + _Env.default.DEFAULT_FADE_OPACITY : "" + 1; //Nodes
  ...
  tick() {
    ...
    this.edgePath.attr("d", l => {
      ...
      if (this.rotateLabels) {
        l.angle = _MathUtils.default.calculateLabelAngle(l.source, l.target);
      }

      let pathStart = _MathUtils.default.calculateIntersection(l.target, l.source, 1);

      let pathEnd = _MathUtils.default.calculateIntersection(l.source, l.target, 1);

      let curvePoint = _MathUtils.default.calculateCurvePoint(pathStart, pathEnd, l);
    ...
    this.activeMultiplicities.attr("transform", function (l) {
      ...
      let n = _MathUtils.default.calculateNormalVector(l.curvePoint, l.source, _Env.default.MULTIPLICITY_VDISTANCE);
