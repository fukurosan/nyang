...
const curveFunction = d3.line().x(function (d) {
  return d.x;
}).y(function (d) {
  return d.y;
}).curve(d3.curveCardinal);
const loopFunction = d3.line().x(function (d) {
  return d.x;
}).y(function (d) {
  return d.y;
...
const calculateIntersection = (source, target, additionalDistance) => {
  ...
  let innerDistance = target.radius; //Rectangles require some more work...
...
const calculateLabelAngle = (point1, point2) => {
  ...
  let angle = theta * (180 / Math.PI); //Convert to a 360 scale
...
const calculateSelfEdgePath = l => {
  const node = l.source;
  const loopShiftAngle = 360 / l.selfCycleCount;
  const loopAngle = Math.min(60, loopShiftAngle * 0.8);
  const arcFrom = calculateRadian(loopShiftAngle * l.selfCycleIndex);
  const arcTo = calculateRadian(loopShiftAngle * l.selfCycleIndex + loopAngle);
